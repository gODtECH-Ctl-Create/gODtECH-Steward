...
function result(findings) {
  return {
    ...
    durationMs: 12,
    git: { isRepository: true, branch: "main", commit: "0123456789abcdef0123456789abcdef01234567", dirty: false, trackedFiles: new Set(["README.md", "src/app.ts"]) },
...
test("Forge evidence contains only safe observed finding fields", () => {
  ...
  assert.deepEqual(evidence.observation.findings[0], { id: secret.id, rule: secret.rule, category: secret.category, severity: secret.severity, path: secret.path, line: secret.line, fingerprint: secret.fingerprint });
