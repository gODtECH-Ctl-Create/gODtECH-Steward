...
export const STEWARD_MANIFEST_SCHEMA_VERSION = 1;
export const STEWARD_RULE_API_VERSION = 1;
export const STEWARD_RESULT_SCHEMA_VERSION = 1;
function isRecord(value) { return typeof value === "object" && value !== null && !Array.isArray(value); }
function assertManifest(condition, message) { if (!condition) throw new Error(`Invalid rule-pack manifest: ${message}`); }
export function parseRulePackManifest(raw) {
...
export function manifestSigningBytes(manifest) { return Buffer.from(canonicalise(manifest), "utf8"); }
...
export async function verifyRulePack(manifestPath, artifactPath, policy) {
  const absoluteManifest = resolve(manifestPath);
  const absoluteArtifact = resolve(artifactPath);
  const manifest = parseRulePackManifest(JSON.parse(await readFile(absoluteManifest, "utf8")));
  ...
  if (metadata.size !== manifest.artifact.sizeBytes) throw new Error(`Artifact size mismatch: manifest=${manifest.artifact.sizeBytes}, actual=${metadata.size}.`);
  const digest = sha256(artifact);
  if (digest.toLowerCase() !== manifest.artifact.sha256.toLowerCase()) throw new Error(`Artifact SHA-256 mismatch: manifest=${manifest.artifact.sha256}, actual=${digest}.`);
  ...
  return { manifest, artifactPath: absoluteArtifact, artifactSha256: digest, sizeBytes: metadata.size, signatureVerified, sandboxEligible: false };
}
