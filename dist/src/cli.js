...
function printRules(disabledPacks, disabledRules) {
...
async function readPreviousReport(root, reportPath) {
  const candidate = JSON.parse(await readFile(resolve(root, reportPath), "utf8"));
  if (!isScanResult(candidate)) throw new Error(`Comparison report is not a valid gODtECH Steward scan result: ${reportPath}`);
  return candidate;
}
async function verifyPackCommand(args) {
  ...
  const output = {
    ...
    artifact: { path: verified.artifactPath, sizeBytes: verified.sizeBytes, sha256: verified.artifactSha256, format: verified.manifest.artifact.format },
...
async function main() {
  ...
  if (!["scan", "doctor", "report", "forge-evidence", "rules", "fix"].includes(command)) throw new Error(`Unknown command: ${command}\n\n${usage()}`);
  const configResult = await loadConfig(root);
  if (command === "rules") {
  ...
  } else if (args.includes("--json")) console.log(toJson(result));
  else console.log(toText(result));
  if (args.includes("--ci")) {
    const failOn = configResult.config.ci.failOn;
